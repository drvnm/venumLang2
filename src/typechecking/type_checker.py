...
class TypeEnv:
    ...
    def define_var(self, tok: Token, type: tokens):
        name = tok.lexeme
        assert_error(name not in self.variables, tok, "Variable '{}' already defined.".format(name))
    ...
    def define_func(self, tok: Token, type: tokens):
        name = tok.lexeme
        assert_error(name not in self.functions, tok, "Function '{}' already defined.".format(name))
...
class TypeChecker:
    ...
    def visit_unary_expr(self, expr: UnaryExpr):
        ...
        assert_error(
            type in acceptable_types[tokens.INT], expr.right.value,
    ...
    def visit_binary_expr(self, expr: BinaryExpr):
        ...
        assert_error(
            left_type in acceptable_types[tokens.INT], expr.operator,
    ...
    def visit_var_stmt(self, var_stmt: VarStmt):
        if var_stmt.expr: # if the variable has an initializer
            ...
            assert_error(
                initializer_type in acceptable_types[var_stmt.type.type], var_stmt.name,
    ...
    def visit_assignment_expr(self, assignment_expr: AssignmentExpr):
        ...
        assert_error(
            value_type in acceptable_types[original_type], assignment_expr.name,
    ...
    def visit_print_stmt(self, print_stmt: PrintStmt):
        ...
        type = self.type_stack.pop()
        assert_error(type in acceptable_types[tokens.INT], "Oops", f"Type {type} is not valid for print statement")
    ...
    def visit_syscall_stmt(self, syscall_stmt: SyscallStmt):
        for expr in syscall_stmt.args:
            ...
            type = self.type_stack.pop()
            assert_error(
                type in (*acceptable_types[tokens.INT], tokens.STRING, tokens.STR), expr.name,
    ...
    def visit_if_stmt(self, if_stmt: IfStmt):
        ...
        assert_error(
            condition_type == tokens.INT, if_stmt.condition.value,
        ...
        for statement in if_stmt.elif_statements:
            ...
            assert_error(
                condition_type in acceptable_types[tokens.INT], cond.value,
                f"Type {condition_type} is not valid for if statement condition"
            )
    ...
    def visit_while_stmt(self, while_stmt: WhileStmt):
        ...
        condition_type = self.type_stack.pop()
        assert_error(
            condition_type == tokens.INT, while_stmt.condition.value,
    ...
    def visit_call_expr(self, call_expr: CallExpr):
        ...
        assert_error(
            callee is not None, call_expr.callee.name,
            f"Function {call_expr.callee.lexeme} is not defined"
        )
        assert_error(
            len(call_expr.arguments) == len(callee.parameters), call_expr.callee.name,
    ...
    def visit_return_stmt(self, return_stmt: ReturnStmt):
        if return_stmt.value:
            ...
            assert_error(
                func_obj.return_type != tokens.VOID, return_stmt.enclosing_func,
                f"Function {return_stmt.enclosing_func.lexeme} does not return a value, but invalid return type was found"
            )
            assert_error(
                value_type in acceptable_types[func_obj.return_type], return_stmt.enclosing_func,
    ...
    def visit_array_stmt(self, array_stmt: ArrayStmt):
        ...
        if array_stmt.exprs:
            for expr in array_stmt.exprs:
                ...
                assert_error(
                    expr_type in acceptable_types[array_stmt.type.type], array_stmt.name,
    ...
    def visit_array_access_expr(self, array_access_expr: ArrayAccessExpr):
        ...
        assert_error(
            index_type == tokens.INT, array_access_expr.index.value,
